...
import { api, APIError, formatNaira, STATUS_LABEL, LIVENESS_TIERS } from './lib/api.js';
import { renderCodeDrawer } from './lib/codeSnippet.js';
import { buildLanguageSnippets, LOOKUP_CALL, VERIFY_PAYOUT_CALL, VERIFY_PAYOUT_BVN_CALL, companyLookupCurlExample, companyLookupTsExample, companyLookupPythonExample, companyLookupGoExample, bulkIdentifyCurlExample, bulkIdentifyTsExample } from './lib/apiExamples.js';
import { autoStartTourIfFirstTime } from './lib/tour.js';
...
function renderHero() {
    ...
    const nameEls = Array.from(document.querySelectorAll('[data-role="player-name"]'));
    nameEls.forEach((el) => (el.textContent = fullName));
    const balanceEls = Array.from(document.querySelectorAll('[data-role="balance"]'));
    balanceEls.forEach((el) => (el.textContent = formatNaira(player.balance_kobo)));
    const winningsEls = Array.from(document.querySelectorAll('[data-role="winnings"]'));
    ...
    if (ageEl)
        ageEl.textContent = player.age ? String(player.age) : '49';
    // Photos
...
function renderOddsButtons() {
    document.querySelectorAll('.sb-odd').forEach((btn) => {
...
function renderBiometricStatus() {
    ...
    const scoreVal = document.getElementById('face-score-val');
    const timestampEl = document.getElementById('face-timestamp');
    ...
    // Security policy
    const toggle = document.getElementById('require-face-toggle');
    if (toggle)
        toggle.checked = player.require_face_for_payout;
    ...
    if (securityStatus) {
        if (!player.require_face_for_payout) {
            ...
            const meets = player.face_verification_status === 'passed' &&
...
function pollForVerificationOutcome() {
    ...
    pollHandle = setInterval(async () => {
        ...
        if (player && player.face_verification_status !== before && player.face_verification_status !== 'pending') {
            showNote(player.face_verification_status === 'passed'
...
// CAC DUE DILIGENCE
// =============================================================================
async function doCACLookup(advanced) {
    const rcInput = document.getElementById('cac_rc_number');
    const rc = rcInput.value.trim() || '0000000';
    const btn = document.getElementById(advanced ? 'btn-cac-advanced' : 'btn-cac-basic');
    const resultBox = document.getElementById('cac-result-box');
    btn.disabled = true;
...
async function init() {
    ...
    if (beneficiaryInput && player)
        beneficiaryInput.value = `${player.first_name} ${player.last_name}`;
    const bankNameInput = document.getElementById('bank_name');
    const payoutBankInput = document.getElementById('payout_bank_name');
    if (player?.saved_bank_name) {
        if (bankNameInput)
            bankNameInput.value = player.saved_bank_name;
        if (payoutBankInput)
            payoutBankInput.value = player.saved_bank_name;
    }
    const accountNumInput = document.getElementById('account_number');
    const payoutAccountInput = document.getElementById('payout_account_number');
    if (player?.saved_account_number) {
        if (accountNumInput)
            accountNumInput.value = player.saved_account_number;
        if (payoutAccountInput)
            payoutAccountInput.value = player.saved_account_number;
    }
    await loadHistory();
    await loadWebhookDeliveries();
    initCodeDrawers();
    ...
    if (faceStatus === 'failed' || faceStatus === 'declined' || faceStatus === 'abandoned') {
        ...
        await refreshPlayer();
        showErrorModal('Biometric Facial Verification Failed: The live selfie did not match your authoritative NIN registry portrait or failed liveness detection. Payouts remain locked under Anti-ATO defense policy.', 'Biometric Facial Verification Failed');
    ...
    document.querySelectorAll('.sb-odd').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (!fixture)
                return;
            selection = { fixture, pick: btn.dataset.pick };
            renderOddsButtons();
            renderBetslip();
        });
    });
    document.querySelectorAll('#league-list button').forEach((btn) => {
        btn.addEventListener('click', () => {
            const league = btn.dataset.league;
            document.querySelectorAll('#league-list button').forEach((b) => b.classList.toggle('sb-league-active', b === btn));
            document.querySelectorAll('.sb-league-group').forEach((section) => {
    ...
    bankForm?.addEventListener('submit', async (e) => {
        ...
        const accountNum = document.getElementById('account_number').value.trim();
        ...
        try {
            ...
            if (payoutBankInput)
                payoutBankInput.value = bankName;
            if (payoutAccountInput)
                payoutAccountInput.value = accountNum;
            if (bvnInput)
                bvnInput.value = bvn;
            await refreshPlayer();
    ...
    // ---------------------------------------------------------------------------
    const startFaceBtn = document.getElementById('start-face-btn');
    ...
    startFaceBtn?.addEventListener('click', async () => {
        ...
        try {
            const betIdInput = document.getElementById('user_bet_id_input');
    ...
    saveSecurityBtn?.addEventListener('click', async () => {
        ...
        try {
            const requireFace = document.getElementById('require-face-toggle').checked;
    ...
    async function executeWithdrawal() {
        ...
        if (player.winnings_kobo <= 0) {
            showErrorModal('Non-Withdrawable Welcome Bonus: The ₦100,000 welcome credit is a wagering bonus and cannot be withdrawn directly. Under NLRC regulations and anti-bonus-farming controls, you must wager on a match and generate winnings first. Click "⚡ Simulate Match Win (+₦250,000)" to win a ticket and unlock withdrawable cash winnings!', 'Winnings Balance Required');
            return;
        }
        ...
        if (player.require_face_for_payout || player.face_verification_status !== 'passed') {
            if (player.face_verification_status === 'failed') {
                showErrorModal('Biometric 2-Step Auth Failed: The last facial verification attempt scored 2% (below threshold) and did not match the authoritative NIN registry portrait. Payout blocked under Anti-ATO defense policy.', 'Biometric Verification Failed');
                return;
            }
            if (player.face_verification_status !== 'passed') {
                showErrorModal('Layer-2 2-Step Auth Required: High-stakes withdrawals require biometric facial verification to ensure the player initiating the payout is the verified NIN owner. Generating your single-use webcam link carrying your user betID now...', 'Biometric 2FA Verification Required');
    ...
    if (player?.face_verification_status === 'pending')
        pollForVerificationOutcome();
}
init();
