import { api, formatNaira, APIError, STATUS_LABEL, LIVENESS_TIERS } from './lib/api.js';
...
function renderFaceStatus() {
    ...
    el.hidden = false;
    const badgeClass = player.face_verification_status === 'passed'
        ? 'verified'
...
const tierButtons = Array.from(document.querySelectorAll('.tier-option'));
...
async function refresh() {
    ...
    document.getElementById('bank_name').value = player.saved_bank_name ?? 'GTBank';
    document.getElementById('account_number').value = player.saved_account_number ?? '';
}
...
depositForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        await api.deposit({ amount_naira: amount, method: document.getElementById('deposit_method').value });
...
const simFacePassBtn = document.getElementById('sim-face-pass-btn');
const simFaceFailBtn = document.getElementById('sim-face-fail-btn');
