import { api, APIError, STATUS_LABEL } from './lib/api.js';
import { renderLanguageTabs } from './lib/codeSnippet.js';
import { showErrorModal } from './lib/modal.js';
import { buildLanguageSnippets, ONBOARD_CALL } from './lib/apiExamples.js';
...
function renderResult(customer, message) {
    let html = `
    ...
    ${(customer.fields ?? []).length === 0
      ...
      <tbody>
        ${(customer.fields ?? [])
            .map((f) => `<tr><td>${escapeHtml(f.field)}</td><td>${escapeHtml(f.provided)}</td><td>${escapeHtml(f.match)}</td><td>${Math.round(f.score * 100)}%</td></tr>`)
            .join('')}
      </tbody>
    </table>`}
