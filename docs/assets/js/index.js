...
function init() {
    ...
    window.addEventListener('ninjabet:navigate-mode', (e) => {
        if (e.detail?.mode) {
...
function bindFintechFlow() {
    const fn = document.getElementById('ft-first-name');
    const ln = document.getElementById('ft-last-name');
    const idType = document.getElementById('ft-id-type');
    const idNum = document.getElementById('ft-id-number');
    const dob = document.getElementById('ft-dob');
    const form = document.getElementById('form-fintech');
...
function executeFintechVerification(p) {
    ...
    if (!result)
        return;
    let fnScore = 0.12;
    if (p.firstName.toLowerCase() === 'james')
        fnScore = 1.0;
    else if (p.firstName.toLowerCase() === 'jams')
        fnScore = 0.88;
    let lnScore = 0.10;
    if (p.lastName.toLowerCase() === 'bond')
        lnScore = 1.0;
    let dobScore = 0.0;
    if (p.dobVal === '1975-01-01')
        dobScore = 1.0;
...
function updateDevCode() {
    ...
    if (currentMode === 'fintech') {
        ...
        const fn = document.getElementById('ft-first-name')?.value || 'James';
        const ln = document.getElementById('ft-last-name')?.value || 'Bond';
        const docType = document.getElementById('ft-id-type')?.value || 'nin';
        const docNumber = document.getElementById('ft-id-number')?.value || '77777777777';
        const dob = document.getElementById('ft-dob')?.value || '1975-01-01';
        if (activeTab === 'curl') {
            lang = 'bash';
